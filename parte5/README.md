...
```
┌───────────────────────────────────────────────────────┐
│  CLÚSTER Kind (nodo único local)                      │
│  ┌────────────┐   ┌────────────┐   ┌──────────────┐  │
│  │ NAMESPACE  │   │  POD       │   │  DEPLOYMENT  │  │
│  │ prestaflow │   │  billetera │   │   réplicas:2 │  │
│  │            │   │  (contenedor)│ │  (declara el │  │
│  └────────────┘   └────────────┘   │   estado)    │  │
│                                    └──────────────┘  │
│  SERVICE (ruta estable hacia los pods)                │
│  CONFIGMAP (config) + SECRET (credenciales)          │
│  HPA (autoescalado por CPU/HTTP)                     │
└───────────────────────────────────────────────────────┘
```
...
┌───────────────────── git push → main ─────────────────────┐
│                                                           │
│  CI (calidad)                    CD (entrega)             │
│  ────────────                    ────────────             │
│  php -l + phpunit + behat       construir imagen prod     │
│  + stage + infection + stan     push a GHCR               │
│  + fix-check (todo de Parte 4)  kubectl apply manifests   │
│                                                           │
│  Jobs en paralelo:              Jobs secuenciales:        │
│  ┌──────┐ ┌────────┐           build → push → deploy     │
│  │ lint │ │ tests  │                                      │
│  └──────┘ └────────┘                                      │
└───────────────────────────────────────────────────────────┘
